...
from models.transformer.utils import sinusoid_encoding_table,get_pretrained_encoding, PositionWiseFeedForward, get_embedding_matrix
...
# import gensim
# model = gensim.models.KeyedVectors.load_word2vec_format('/content/drive/MyDrive/Quan_task/embedded_M2forviic/word2vec_vi_words_300dims.txt')
...
class MeshedDecoder(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1,
        ...
        #self.word_emb = nn.Embedding.from_pretrained(get_pretrained_encoding('/content/drive/MyDrive/Quan_task/embedded_M2forviic/word2vec_vi_words_300dims.txt'))
        self.word_emb = nn.Embedding(vocab_size, d_model, padding_idx=padding_idx)
        self.word_emb.weight = nn.Parameter(get_embedding_matrix())
        self.word_emb.weight.requires_grad=False
        #self.word_emb = nn.Embedding.from_pretrained(get_embedding_matrix())
    ...
    def forward(self, input, encoder_output, mask_encoder):
        ...
        #print(seq.shape)
        #print(max(max(x) for x in list(seq)))
