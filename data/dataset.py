...
class COCO(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=False,
                 cut_validation=False,splits=["train", "val", "test"]):
        roots = {}
        '''
        roots['train'] = {
        ...
        if id_root is not None:
            ...
            print(ids['train'])
            print('\n')
            # with open('/content/drive/MyDrive/Caption_data/m2_feature/annotations/uitviic_captions_train2017.json','r') as f1:
            #     data1 = json.load(f1)
            # ids_train = []
            # for i in range(len(data1['images'])):
            #     idt = data1['images'][i]['id']
            #     ids_train.append(idt)
            # ids['train'] = ids_train
            #ids['val'] = np.load(os.path.join(id_root, 'uitviic_val_ids.npy'),allow_pickle=True)
            with open('/content/drive/MyDrive/Quan_task/embedded_M2forviic/annotations/uitviic_captions_val2017.json','r') as f:
                data = json.load(f)
            ids_val = []
            for i in range(len(data['annotations'])):
                idt = data['annotations'][i]['id']
                ids_val.append(idt)
            ...
            # with open('/content/drive/MyDrive/Caption_data/m2_feature/annotations/uitviic_captions_test2017.json','r') as f2:
            #     data2 = json.load(f2)
            # ids_test = []
            # for i in range(len(data2['images'])):
            #     idt = data2['images'][i]['id']
            #     ids_test.append(idt)
            # ids['test'] = ids_test
            '''
            ids['trainrestval'] = (
